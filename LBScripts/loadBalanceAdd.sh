...
then
	if [ $5 -ne 0 ];
	then
		ip netns exec $6 iptables -t nat -I PREROUTING 1 -p tcp -d $2 --dport $3 -m state --state NEW -j DNAT --to-destination $4:$5
	else
		ip netns exec $6 iptables -t nat -I PREROUTING 1 -p tcp -d $2 --dport $3 -m state --state NEW -j DNAT --to-destination $4
	fi
	exit
fi
