...
if flag == 0:
    ...
    for x in schema['subnet_list2']:
        subprocess.call(shlex.split('./addRouteToSubnet.sh ' +schema['ns_name'] +' '+x['subnet']+'/'+str(x['prefix'])+' ' +schema['gre_name']))
    ...
    for k in schema['subnet_list2']:
            ...
            last_ip_range = get_ip(k['subnet'],240)
            subnet_name = k['name']
            subprocess.call(shlex.split('/home/ece792/AutoScalingAsAService/addTenantSubnet.sh '+schema['ns_name']+' '+k['subnet']+' '+str(k['prefix'])+' '+first_ip_range+' '+last_ip_range+' '+k['base_ns_subnet']))
    ...
    for x in schema['subnet_list1']:
         subprocess.call(shlex.split('/home/ece792/AutoScalingAsAService/addRouteToSubnet.sh ' +schema['ns_name'] +' '+x['subnet']+'/'+str(x['prefix'])+' ' +schema['gre_name']))
